...
func findClosest(words []string, word1 string, word2 string) int {
	...
	for ind, s := range words {
		wordMap[s] = append(wordMap[s], ind)
	}
	len1, len2, ind1, ind2 := len(wordMap[word1]), len(wordMap[word2]), 0, 0
	ans := len(words)
	for ind1 < len1 && ind2 < len2 {
		v := wordMap[word1][ind1] - wordMap[word2][ind2]
		if v > 0 {
			if v < ans {
				ans = v
			}
